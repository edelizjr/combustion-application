...
import com.reboticreg.myapp.config.KafkaSseConsumer;
import com.reboticreg.myapp.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class CombustionKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/combustion-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/combustion-kafka/consume")
